...
excitation_type = 'near_fault';        % 'artificial_seismic_wave','white_noise', 'near_fault'
input_format = 'IM';                                % 'IM', 'time_history'
input_name = 'near_fault_input';                          % 'arti_input','arti_input_4', 'white_input_timehis','near_fault_input_linear'
                                                               % 'white_input_nonlinear'
output_name = 'near_fault_F_envelop';                    %  'arti_F_envelop','white_F_base_timehis',...
                                                    %  'near_fault_F_linear_envelop'
                                                    
                                                    %  'near_fault_M_envelop','near_fault_M_base_timehis',...
                                                    
                                                     %  'arti_CD_base_max_4','near_fault_CD_base_max','near_fault_CD_envelop'
                                                    %  'near_fault_CD_base_timehis',...
                                                    % 'white_CD_base_max_nonlinear'
                                                    
                                                    %  'near_fault_Dis_base_timehis')
...
path = ['C:\Users\cxtj1\Desktop\matlab_fragility\Paper1_Nonlinear_response\A_ML_results\' 'LSTM\' 'white_noise\' document_name];  


%% ================ Step 1.2 Pre-process for dataset ================

% ==== Determine the Model/ Features ====
options.ML_name = 'LSTM';                            % 'ANN','LSTM'
options.problem_type = 'regression';                 % 'classification', 'regression', 'class_regression'
options.feature_selecte = 'all';               % 'arti_type'/ 'white_type'/ 'all' (determine the input variables)
...
options.amount = 500;                                % the data for training/test process; while the remaining corre. to predict
...
model_amount = 10;                                   % the total amount of trained models, and the final results corre. their average values.


%% ======== Step 1.4 specify hyper-para. of machine learning ========= 

AA_two_trail_analysis = [];                                      % employed for saving the results of the two 'for' repeate process
AA_two_train_trail_analysis = [];
AA_two_test_trail_analysis = [];
AA_two_pre_trail_analysis = [];


LSTM_type = 'sequence';                                          % 'sequence' or 'last' 
numHiddenUnits = 330;                                             % specify the hidden neurons
maxEpochs = 400;
miniBatchSize = 20 ;
...
for block = 1                       % 不需要这一for循环时，则设置为block
...
featureDimension = size(data_summary.train_input{1},1);         %  此处与ANN不一样
if iscell(data_summary.train_output)
    numResponses = size(data_summary.train_output{1},1);        % 当为cell格式时；
else
    numResponses = size(data_summary.train_output(1,:),1);      % 当为numeric格式时；
end
...
layers = [ ...
    ...
    lstmLayer(numHiddenUnits,'OutputMode',LSTM_type)
...
if strcmp(process_type,'training_process')
    ...
    for epoch_num = 1:model_amount
        disp([num2str(epoch_num) 'th model developed'])
        ...
        predict_x = data_summary.predict_input;
        predict_y = data_summary.predict_output;
...
end % the first 'for' repeate 


%% ============== Summary of the different output cases =============
    if strcmp(process_type,'cross_validate_process')
        AA_two_trail_analysis = cat(3,AA_two_trail_analysis,AA_train_trail_analysis);

    
    else
        AA_two_train_trail_analysis = cat(3,AA_two_train_trail_analysis,AA_train_trail_analysis);
        AA_two_test_trail_analysis = cat(3,AA_two_test_trail_analysis,AA_test_trail_analysis);
        AA_two_pre_trail_analysis = cat(3,AA_two_pre_trail_analysis,AA_pre_trail_analysis);
        
    end

    disp(['summary size is' num2str(size(AA_two_trail_analysis) )  ] )
